...
def investment_requirement(request):
    """Handle Send Your Requirements submission"""
    try:
        data = {
            'name': request.POST.get('name'),
            'email': request.POST.get('email'),
...
def requirements_submission(request):
    ...
    if request.method == 'POST':
        try:
            ...
            logger.info(f'Requirements submission POST data: {dict(request.POST)}')
            ...
            # Budget handling - can come from either budget or budget_range field
            budget = request.POST.get('budget', '')
            ...
            logger.info(f'Extracted data - Name: {name}, Contact: {contact_number}, Type: {requirement_type}, Property: {property_type}')
            logger.info(f'Budget: {budget}, Budget Range: {budget_range}, Enquiry From: {enquiry_from}')
            
            # Validate required fields
            if not name or not contact_number:
                messages.error(request, 'Please fill in all required fields (Name and Contact Number).')
            ...
            logger.info(f'Successfully created requirement with ID: {requirement.id}')
            ...
            import logging
            logger = logging.getLogger(__name__)
            logger.error(f'Error in requirements_submission: {str(e)}')
            
            messages.error(request, f'Sorry, there was an error submitting your requirement: {str(e)}. Please try again.')
            return redirect('theme:requirements_page')
