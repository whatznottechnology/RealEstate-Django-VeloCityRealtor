...
class NearestAreaInline(admin.TabularInline):
    ...
    fields = ('name', 'area_type', 'distance')
...
class ConstructionUpdateInline(admin.TabularInline):
    ...
    fields = ('image_preview', 'title', 'image', 'description', 'update_date', 'order')
    readonly_fields = ('image_preview',)
...
class ProjectAmenityImageInline(admin.TabularInline):
    ...
    fields = ('image_preview', 'amenity_name', 'image', 'is_active', 'uploaded_at')
    readonly_fields = ('uploaded_at', 'image_preview')
...
class ProjectAdmin(admin.ModelAdmin):
    ...
    def project_thumbnail(self, obj):
        ...
        return format_html('<div style="width:60px;height:40px;background:#f0f0f0;border-radius:4px;display:flex;align-items:center;justify-content:center;font-size:10px;color:#666;">No Image</div>')
    ...
    def feature_badges(self, obj):
        ...
        if badges:
            return format_html('<div class="feature-badges-container">{}</div>', ''.join(badges))
        return format_html('<span class="no-features">No features</span>')
...
admin.site.site_header = "🏢 Real Estate Project Management"
admin.site.site_title = "Real Estate Admin"
admin.site.index_title = "Welcome to Real Estate Dashboard"
