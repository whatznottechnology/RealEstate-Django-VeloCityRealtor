...
class Command(BaseCommand):
    ...
    def create_tags(self):
        ...
        for tag_data in tags_data:
            Tag.objects.get_or_create(name=tag_data['name'], defaults=tag_data)
    ...
    def create_projects(self):
        ...
        for project_data in projects_data:
            ...
            project_type_queryset = ProjectType.objects.filter(category=category)
            project_type = random.choice(list(project_type_queryset)) if project_type_queryset.exists() else random.choice(project_types)
            ...
            if created:
                ...
                project.tags.set(random.sample(tags, k=min(len(tags), random.randint(2, 4))))
                project.amenities.set(random.sample(amenities, k=min(len(amenities), random.randint(5, 10))))
